...
//start unok's PATHINFO url script
$subpath = substr(stristr($_SERVER['REQUEST_URI'], '.php/'), 5); //this returns the url after "index.php/"
$args = explode('/', $subpath, 3); // we get the arguments in an array; no more than 3 arguments though!
if (array_key_exists(0, $args) && $args[0] != '') $_GET['manga'] = $args[0];
if (array_key_exists(1, $args) && $args[1] != '') $_GET['chapter'] = $args[1];
if (array_key_exists(2, $args) && $args[2] != '') $_GET['page'] = $args[2]; //and we set the GET arguments with them
//end unok's PATHINFO url script
...
function change_manga(manga) {
	if (manga != 0) {
		 document.location = "<?php echo $omv_base_url ?>" + "index.php/" + manga;
	}
}

function change_chapter(manga, chapter) {
	if (manga != 0) {
		document.location = "<?php echo $omv_base_url ?>" + "index.php/" + manga + "/" + chapter;
	}
}

function change_page(manga, chapter, page) {
	if (manga != 0) {
		 document.location = "<?php echo $omv_base_url ?>" + "index.php/" + manga + "/" + chapter + "/" + page;
	}
}
</script>
	...
	if ($chapter) {
		...
		if ($page) {
			...
			if ($page <= 1) {
				...
				$prevhtml = "<a href=\"index.php/$manga_escaped/$chapter_number_escaped/" . ($page - 1) . "\"><img src=\"themes/$theme/previous.png\" alt=\"Previous Page\" title=\"Previous Page\" /></a>";
			}
			$nexthtml = "";
			if ($page >= count($pages)) {
				$nexthtml = "<img src=\"themes/$theme/no-next.png\" alt=\"\" />";
			} else {
				$nexthtml = "<a href=\"index.php/$manga_escaped/$chapter_number_escaped/" . ($page + 1) . "\"><img src=\"themes/$theme/next.png\" alt=\"Next Page\" title=\"Next Page\" /></a>";
			}
...
if ($manga) {
	if ($chapter) {
		if ($page) {
			...
			if ($next_page_path) {
				  $imghtml = "<a href=\"index.php/$next_page_path\">" . $imghtml . "</a>";
			}
			echo $imghtml;
		} else {
...
?>
		case 37:
		window.location = "<?php echo $omv_base_url . 'index.php/' . $prev_page_path ?>";
		return false;
...
?>
		case 39:
                window.location = "<?php echo $omv_base_url . 'index.php/' . $next_page_path ?>";
		return false;
